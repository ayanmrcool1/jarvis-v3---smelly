...
def open_application(app_name):
    ...
    try:
        ...
        # If user directly says a website
        if "." in clean_name:
            url = clean_name
...
def search_web(query):
    ...
    Later we can upgrade this to return scraped results.
    """
...
def run_terminal_command(command, timeout_seconds=10):
    ...
    Note:
    This can be powerful, so later when AI tool calling is connected,
    we will add extra protection for destructive commands.
    """
...
def set_volume(action):
    ...
    try:
        ...
        if clean_action == "up":
            ...
            return {
                "success": True,
                "message": f"Volume increased from {int(current_volume * 100)}% to {int(new_volume * 100)}%.",
            }
        ...
        if clean_action == "down":
            ...
            return {
                "success": True,
                "message": f"Volume decreased from {int(current_volume * 100)}% to {int(new_volume * 100)}%.",
            }
        ...
        if clean_action == "mute":
            ...
            return {
                "success": True,
                "message": "Volume muted." if new_mute else "Volume unmuted.",
            }

        return {
            "success": False,
            "message": "Unsupported volume action. Use up, down, or mute.",
        }
